...
def calibrate_model(model, dataset, config):
  with torch.no_grad():
    ...
    if config['dataset'] == 'temca':
      ...
      DL_queue = [...] # define your queue of calibration sets
      list_of_labels = [labels for i in range(len(DL_queue))]
      list_of_outputs = [outputs for i in range(len(DL_queue))]
      for i, tempDL in enumerate(DL_queue):
        counter = 0
        for batch in tqdm(tempDL):
          list_of_outputs[i][counter:counter+batch[0].shape[0],:,:,:,:] = model(batch[0].to(device)).cpu()
          list_of_labels[i][counter:counter+batch[1].shape[0]] = batch[1]
    ...
    out_datasets = [TensorDataset(list_of_outputs[i],list_of_labels[i].cpu()) for i in range(len(list_of_outputs))]
    ...
    calib_loss_tables = [torch.zeros((list_of_outputs[0].shape[0],lambdas.shape[0]))]
    for lam in reversed(lambdas):
      for i, out_dataset in enumerate(out_datasets):
        ...
        calib_loss_tables[i][:,np.where(lambdas==lam)[0]] = losses[:,None]
        Rhat = losses.mean()
        ...
        if Rhat >= alpha or RhatPlus > alpha:
          ...
          break
    return model, calib_loss_tables
